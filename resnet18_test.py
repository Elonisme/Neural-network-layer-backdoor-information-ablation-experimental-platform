...
def mixing_model(clear_weights, poison_weights, layer_name1, layer_name2):
    mixture_model = copy.deepcopy(clear_weights)
    for key_name in clear_weights:
        if layer_name1 in key_name and layer_name2 not in key_name:
            print(key_name)
...
if __name__ == '__main__':
    ...
    # 初始化模型、损失函数和优化器
    clear_model = resnet18().to(device)
    ...
    mix_model = resnet18().to(device)
    mix_model_weights = mixing_model(clear_model_weights, poison_model_weights, layer_name1="fc", layer_name2='conv')
    ...
    mix_model_weights = mixing_model(clear_model_weights, poison_model_weights, layer_name1="conv", layer_name2="other")
    mix_model.load_state_dict(mix_model_weights)
    ...
    categories = ['Ma Of Clear Model', 'Ba Of Clear Model',
                  ...
                  "Ma Of Fc Mixture Model", "Ba Of Fc Mixture Model",
    ...
    # 添加标题和标签
    plt.title('Robustness testing for various Resnet18 models')
    plt.ylabel('Accuracy')
    ...
    plt.tight_layout()
    plt.savefig('Robustness_testing_for_various_resnet18_models.png')
    # 显示柱状图
    plt.show()
