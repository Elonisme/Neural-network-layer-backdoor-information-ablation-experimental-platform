...
from model.cifar10vgg import VGG
...
def get_vgg_model(device):
    clear_model = VGG('VGG16').to(device)
    poison_model = VGG('VGG16').to(device)
    return poison_model, clear_model


def vgg_robustness_test(
        clear_model,
        poison_model,
        train_loader,
...
def vgg16_backdoor_information_detect():
    ...
    # 初始化模型、损失函数和优化器
    poison_model, clear_model = get_vgg_model(device=device)
    values = vgg_robustness_test(
    ...
    save_pairs_to_csv(file_path="csv/vgg16.csv", values=values)
    save_bar(values=values, net_name="vgg16")
