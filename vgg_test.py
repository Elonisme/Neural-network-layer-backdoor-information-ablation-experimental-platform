...
def get_vgg_model(device):
    clear_model = VGG('VGG13').to(device)
    poison_model = VGG('VGG13').to(device)
...
def vgg_backdoor_information_detect():
    ...
    save_pairs_to_csv(file_path="csv/vgg13.csv", values=values)
    save_bar(values=values, net_name="vgg13")
